...
Let me know if you'd like help adding badges (e.g. build status), generating a plugin hub submission snippet, or auditing code coverage!
::contentReference[oaicite:0]{index=0}
# Hopper

## Overview

**Hopper** is a RuneLite plugin that tracks your world-hops using a token bucket system. By default, it supports a hop cap of 400 and regenerates about 93 hops per hour. The plugin provides a live overlay that shows your remaining hops, how many are needed to refill, and an estimated countdown until full capacity.
...
1. Enable the **Hopper** plugin in RuneLite’s sidebar.
2. Monitor the overlay:
    - **Hops** show current vs maximum.
    - **To full** indicates how many hops needed to refill.
    - **Full in** counts down until you're full (updates each tick).
3. Configure warnings and behavior via the settings panel.
